...
# List of {q, correct_a, user_a} dicts
...
# Stores options for the currently displayed question
QK_CURRENT_OPTIONS = 'current_options'
...
def _get_default_quiz_state(num_questions: int) -> Dict[str, Any]:
    ...
    return {
        ...
        # Stores the user's selected answer for each question {index: answer}
        QK_USER_ANSWERS: {},
        # Flag indicating if the current question's answer was submitted
        QK_SUBMITTED: False,
...
def load_flashcards(uploaded_file) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        df = df.rename(
            columns={df.columns[0]: COL_QUESTION, df.columns[1]: COL_ANSWER})
        # Ensure columns exist after renaming
        if COL_QUESTION not in df.columns or COL_ANSWER not in df.columns:
            st.error(
                f"Could not find required columns '{COL_QUESTION}' and '{COL_ANSWER}' after renaming.")
            return None
...
def generate_options(correct_answer: str, all_answers: List[str], num_options: int = NUM_OPTIONS) -> List[str]:
    ...
    other_answers = [
        ans for ans in all_answers if ans.strip().lower() != correct_lower]

    # Ensure enough unique incorrect options are available
    num_incorrect_needed = num_options - 1
    num_available_incorrect = len(
        set(ans.strip() for ans in other_answers))  # Count unique incorrect

    if num_available_incorrect >= num_incorrect_needed:
        # Sample unique incorrect answers
        incorrect_options_pool = list(
            set(ans.strip() for ans in other_answers))
        incorrect_options = random.sample(
            incorrect_options_pool, num_incorrect_needed)
...
def handle_submit(current_card_index: int):
    ...
    if check_answer(user_answer, current_card[COL_ANSWER]):
        ...
        st.error(
            f"Incorrect. The correct answer is: {current_card[COL_ANSWER]}")
...
def main():
    ...
    user_name = st.sidebar.text_input(
        "Enter your name to save results:", value=st.session_state[SS_USER])
    if user_name:
        ...
        st.markdown(
            ...
            **Welcome to the Flashcard Quiz App!**
            ...
            2.  **Upload Your Flashcards:**
                *   Under "Load Flashcards" below, click "Browse files".
            ...
            3.  **Start the Quiz:** Once flashcards are loaded, click the "**🚀 Start Quiz**" button in the main area (right side).
            4.  **Answer Questions:**
            ...
            6.  **Restarting:**
                *   **During Quiz:** Use the "**🔁 Restart Quiz Now**" button under "Quiz Controls" below to start over with the same flashcards at any time.
    ...
    uploaded_file = st.sidebar.file_uploader(
        ...
        type=["csv", "xlsx", "xls"],
        # Added help text
        help="Select a CSV or Excel file. Needs at least two columns: Question (col 1) and Answer (col 2)."
    )
    ...
    if uploaded_file is not None:
        ...
        if SS_FLASHCARDS not in st.session_state or current_file_name != previous_file_name:
            ...
            if flashcards:
                ...
                if '_loaded_file_name' in st.session_state:
                    del st.session_state['_loaded_file_name']
                if SS_QUIZ_DATA in st.session_state:
                    del st.session_state[SS_QUIZ_DATA]
                if SS_CURRENT_INDEX in st.session_state:
                    del st.session_state[SS_CURRENT_INDEX]
    ...
    if SS_FLASHCARDS in st.session_state:
        ...
        if 'all_answers' not in st.session_state:
            st.session_state['all_answers'] = list(
                set(card[COL_ANSWER] for card in flashcards))
            ...
            if not available_indices:
                ...
                if not options:
                    options = generate_options(
                        current_card[COL_ANSWER], all_answers)
                    quiz_data[QK_CURRENT_OPTIONS] = options

                # --- Display Question UI ---
                if options:
                    user_answer = display_question(
                        current_card, options, f"q_{current_card_index}")
                else:
